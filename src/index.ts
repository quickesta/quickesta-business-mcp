...
/**
 * Quickesta Business MCP Server
 *
 * Multi-tenant Business data exposed to AI assistants via MCP.
 ...
 *   HASURA_GRAPHQL_URL       — Business Hasura endpoint (required)
 *   HASURA_ADMIN_SECRET      — Admin secret for Business Hasura (required)
...
const HASURA_URL = process.env.HASURA_GRAPHQL_URL || 'https://graphql-business.quickesta.com/v1/graphql'
...
const allTools: ToolDefinition[] = [
  ...createAppointmentTools(hasura),
  ...createCustomerTools(hasura),
  ...createMemberTools(hasura),
  ...createServiceTools(hasura),
...
function createMcpServer(): Server {
  const s = new Server(
    { name: 'quickesta-business', version: '1.0.0' },
    { capabilities: { tools: {} } }
  )
  ...
  s.setRequestHandler(ListToolsRequestSchema, async () => {
    ...
    const myProductsTool = {
      ...
      description: 'Yetkili olduğunuz Business ürünlerini listeler.',
...
async function main() {
  if (TRANSPORT === 'http') {
    ...
    httpServer.listen(PORT, () => {
      console.log(`Quickesta Business MCP Server (HTTP) — port ${PORT}`)
      console.log(`  Sağlık: http://localhost:${PORT}/health`)
      console.log(`  MCP:    http://localhost:${PORT}/mcp`)
      console.log(`  Araçlar: ${allTools.length + 1} (${allTools.length} Business + 1 meta)`)
    })
  } else {
    if (!MCP_API_KEY) {
    ...
    console.error(`Quickesta Business MCP Server (STDIO) — ${allTools.length + 1} araç`)
  }
}
